...
#		'image_notes': 'root.DocumentObjectList.10.Text' # = Image Notes 		
	...
	# else, return false value
		...
		return 0
#Added by William Fox on ---3-24-2014---
#Produces an image file compatible with html browsers
...
#Addition forcreating a grayscale image of the data
def dm3_image_form(dm3_file,im_file,dm3_data_list):
	#dm3_file='T.dm3' 
	makeJPEG=True
	#tn_file='dm3tn_temp.'
	...
	if tags:
		if makeJPEG:
			# get thumbnail
			im_size = int( tags[ 'root.ImageList.0.ImageData.Data.Size' ] )
			im_offset = int( tags[ 'root.ImageList.0.ImageData.Data.Offset' ] )
			im_width = int( tags[ 'root.ImageList.0.ImageData.Dimensions.0' ] )
			im_height = int( tags[ 'root.ImageList.0.ImageData.Dimensions.1' ] )
			
			if ( (im_width*im_height*4) != im_size ):
				print "Error: cannot extract thumbnail from", dm3_file
				sys.exit()
			...
			for i in range(im_height):
				for ii in range(im_width):
					...
					pgmlist[i][ii]=int(color)
			#create thumbnail
			im=Image.new("RGB",(im_width,im_height))#black and white option is L
			im.putpixel((50,50),(255,0,0))
			for i in range(im_height):
				for ii in range(im_width):
					scaled=int(pgmlist[i][ii])
