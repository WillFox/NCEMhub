...
%   - Need: add unit check for X-ray: "eV"
% Modified P Ercius 2013/01
%   - Fixed bug with global variable writeTags which stopped tags from writing out
%%
%Main Function
function scienceData = dm3_reader4(inName,writeTags)

    if nargin == 0
        inName = getfile('*.dm3');
        writeTags = 0;
    elseif nargin == 1
    	writeTags = 0; % do not write out the tags if not requested
    elseif nargin > 1 && writeTags ~= 1
    	writeTags = 0; %only write out tags if writeFlag is specifically 1
    end
    
    %Clear globals
	clear global curGroupLevel MAXDEPTH curGroupAtLevelX curGroupNameAtLevelX;
    clear global curTagAtLevelX curTagName FIDout writeTags;
	...
	global curTagAtLevelX curTagName FIDout writeTags;
	...
	%Error if reading file fails
	if FID == -1
		error(FIDmessage);
	end
	
	%Fail if opening write file fails
	if(writeTags && FIDout == -1)
		error(FIDoutmessage);
	end
...
function arrOut=readArrayData(FID,arrayTypes)
	...
	if(encodedType == 4)
        ...
        %Find the character values larger than zeros to remove the zeros
        letterPositions = find(stringData>0);
        %Return only the characters as a character string
        arrOut = char(stringData(letterPositions))';
        
        %Test for interesting units names
        if(strcmp(arrOut,'nm')) %image
            scale(numObjects) = scale_temp;
        elseif( strcmp(arrOut,'eV')) %EELS spectra
            scale(numObjects) = scale_temp;
            origin(numObjects) = origin_temp;
        elseif(strcmp(arrOut,'keV')) %EDX spectra
        	scale(numObjects) = scale_temp;
        	origin(numObjects) = origin_temp;
        end
...
function storeTag(curTagName,curTagValue)
	...
	global xsize ysize zsize dataType scale_temp origin_temp writeTags;
